...
var vm = new Vue(
    {
        ...
        methods:
        {
            deletem : function (roleId)
            {
                layer.open(
                    {
                        content: '您确定要删除该记录吗？',btn: ['确定', '取消'],yes: function(index)
                    {
                        $.post("/role/delete?roleId=" + roleId, null, function(f)
                        {
                            if(f == 1) {
                            ...
                            layer.open({content : "该角色存在绑定权限，请先解绑！", skin: 'msg', time: 2});
                        }else {
                ...
                $.ajax(
                    {
                        type: "post",
                        url: "/role/getList",
                        data: {pageNo: index, pageSize: this.pagesize, roleName: roleName,roleId:roleId},
